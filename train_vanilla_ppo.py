"""
Training Script for Fuel Collection Environment.
...
def train(timesteps: int = 50000000, seed: int = 42, save_dir: str = "models"):
    """Train the agent."""
    print("="*60)
    print("  Training Fuel Collection Agent")
    print("="*60)
    ...
    log_dir = os.path.join("runs", f"fuel_agent_{timestamp}")
    ...
    agent = PPOAgent(env, config, reward_scheme=None)
    ...
    temp_best_path = os.path.join(save_dir, "best_model.pt")
    model_path = os.path.join(save_dir, f"agent_{timestamp}.pt")
...
def main():
    parser = argparse.ArgumentParser(description='Train Fuel Collection Agent')
